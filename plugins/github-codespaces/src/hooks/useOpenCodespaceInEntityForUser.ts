 ...
 */
export function useOpenCodespaceInEntityForUser(entity: Entity) {
  ...
  const verifyCodespaceNameWithEntity = (
    entityName: string,
    codespaceName: any,
  ) => {
  ...
  const startCodespace = useCallback(async () => {
    ...
    const codespacesListInRepo = await api.listCodespacesInRepoForUser(
      owner,
      repositoryName,
    );
    ...
    return count === 0
      ? await api.createCodespaceInEntityForUser(
          entityName,
          owner,
          repositoryName,
          devcontainerPath,
