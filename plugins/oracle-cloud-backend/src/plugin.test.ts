...
import { oracleCloudPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [oracleCloudPlugin],
    });

    await request(server).get('/api/oracle-cloud/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/oracle-cloud/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/oracle-cloud/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/oracle-cloud/todos/${createdTodoItem.id}`)
